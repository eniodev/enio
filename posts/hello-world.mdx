...
2. The connection can be closed on the client or server side.
    a. Closed on the server side: It can be due to successful completion of the interaction, or internal error, such as a software error or overload, thus issuing an error code to the client.
    b. Closed on the client side: It can be for several reasons such as network error or the receipt of a complete response, in which case no error is issued.
    c. The process of request (from the client to the server) and response (from the server to the client) is called a transaction. A transaction is made using data blocks, called HTTP Messages, that is, requests and responses are HTTP messages.
...
```jsx
N/A //i don't need a body in this case
```
...
1. ### Start Line
    a. Contains the protocol version, followed by the response code and a short descriptive message.
...
2. ### Headers
    a. Contains information related to the response, such as resource type and size.
...
3. ### Body
    a. As with the request, this is where the requested resource is located.
...
And now, what about the resource type definition? Resources are stored on the server, and it can be difficult for the server to understand which resource the client wants if the type is not specified. What would happen if I had the files party.mp3 and party.png on the server, but the request only said that I want the file 'party'? How would the server handle this?
...
For example, [http://home.com/bedroom/table/paper.txt](http://home.com/bedroom/table/paper.txt) shows how the URI would look to access the resource paper.txt.
...
[http://home.com/bedroom/table/paper.txt](http://home.com/bedroom/table/paper.txt) is indeed a URL. URLs are made up of three parts:

1. Schema: http:// (Defines the protocol to be used to access the resource)
2. Domain/IP: [home.com](http://home.com/) (Defines where to look for the resource)
...
For example, urn:example:home.com:bedroom:table:paper.txt.
